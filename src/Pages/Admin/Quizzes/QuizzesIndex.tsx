...
const QuizzesIndex = () => {
    ...
    const fetchQuizzes = async (page = 1) => {
      try {
        const response = await axios.get(
          `/api/quizzes?page=${page}&limit=${quizzesPerPage}`,
          {
  ...
  const addQuiz = async (newQuiz: any, { resetForm }: any) => {
  ...
  const handleSaveQuiz = async (updatedQuiz: any) => {
    ...
    try {
      ...
      setQuizzes((prev) =>
        prev.map((quiz) => (quiz._id === quizId ? response.data.quiz : quiz)),
      );
      setEditingQuiz(null);
      ...
      {showQuizTable ? (
        <>
          <QuizTable
            ...
            onBulkDeleteQuizzes={bulkDeleteQuizzes}
          />
        </>
      ) : editingQuiz ? (
        <div id="editTableMain">
          <QuizCreateForm
            recruiterId={recruiterId}
            quiz={editingQuiz}
            closeForm={closeForm}
            resetForm={resetForm}
            onSubmit={(updatedQuiz) => handleSaveQuiz(updatedQuiz)}
            setShowQuizTable={setShowQuizTable}
          />
        </div>
      ) : (
        <div id="createTableMain">
